...
export default async function ProjectsShowcase({ posts }: ProjectsShowcaseProps) {
  return (
    <div className="w-full bg-white py-12 px-4 sm:px-6 lg:px-20">
      ...
      <div className="mt-6 flex flex-wrap justify-center gap-4 text-black">
        ...
        {['Business Advice','Startup Business','Financial Advice','Risk Management'].map((label) => (
          <div key={label} className="px-4 py-2 hover:bg-gray-100 rounded-md cursor-pointer">{label}</div>
        ))}
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-1 md:grid-cols-2 gap-6 mt-8">
        {posts.map((post: BlogPostWithSections, index: number) => (
          <div key={index} className=" overflow-hidden hover:shadow-lg transition-shadow duration-300">
            <Image src={post.thumbnail as string} alt="post" width={400} height={200} className="w-full h-48 object-cover" />
            <div className="p-4">
              <h2 className="text-black font-semibold mb-2">{post.title}</h2>
              <hr className="border-t border-gray-300 mb-2" />
              <div className="flex justify-between text-gray-500 text-sm">
                <span>{post.created_at ? post.created_at.split("T")[0] : "Unknown"}</span>
                <span className="hover:text-red-600 cursor-pointer">
                  <Link href={`/blog/${post.id}`} className="hover:text-red-600">Read more →</Link>
                </span>
              </div>
            </div>
          </div>
